...
-- Season snapshots (ranking congelado ao finalizar temporada)
...
-- Migration: Add own_goal to event_actions action_type CHECK constraint
-- Run this manually in production:
-- ALTER TABLE event_actions DROP CONSTRAINT event_actions_action_type_check;
-- ALTER TABLE event_actions ADD CONSTRAINT event_actions_action_type_check
--   CHECK (action_type IN ('goal', 'assist', 'save', 'tackle', 'error',
--          'yellow_card', 'red_card', 'period_start', 'period_end', 'own_goal'));

-- Materialized view for event scoreboard (includes own_goal logic)
...
-- Mobile push notification tokens (Capacitor Android/iOS)
